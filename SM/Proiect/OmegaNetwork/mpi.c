...
void check_possibility_mpi(int *version, int io_pair, int *found_result) {
	int shuffled_value;
...
void generate_possibilities_mpi(int *version, int io_pair, int *found_result) {
	if (!(*found_result)) {
        for (int i = 2; i < nr_of_blocks; i++) {
	        if (version[i] == EMPTY) {
	            for (int j = 0; j < BLOCK_TYPES_NR; j++) {
	                ...
	                if (i == nr_of_blocks - 1) {
	                    if (!(*found_result)) {
                            check_possibility_mpi(version, io_pair, found_result);
	                    }
	                } else {
	                    generate_possibilities_mpi(version, io_pair, found_result);
	                }
	            }
	            version[i] = EMPTY;
	            return;
